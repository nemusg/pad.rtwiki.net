...
define('PLUGIN_PCOMMENT_SIZE_MSG',  35);
...
// 0: Update parent page, 1: Update rocording page, 2: Update both
define('PLUGIN_PCOMMENT_TIMESTAMP', 2);

// ----

$cip = preg_replace("/(\d+)\.(\d+)\.(\d+)\.(\d+)/","$4",REMOTE_ADDR);

$str_md5 = substr(md5(REMOTE_ADDR), 0, 30);
//$date_md5 = substr(md5(date("Y-m-d")), 0, 20);
//$key_md5 = substr(md5("rtwikiki"), 0, 10);
...
function plugin_pcomment_convert()
{
	...
	if (PKWK_READONLY) {
		...
		if ($params['noname']) {
			...
			$name = '<input type="hidden" name="name" size="' . PLUGIN_PCOMMENT_SIZE_NAME . '" value=" " />';
		}
		...
		$form_start = '<form name="commentForm" action="' . get_script_uri() . '" method="post">' . "\n";
		$form = <<<EOD
  <div>
  ...
  <label onclick=""><input name="agreecheck" type="checkbox" onclick="changeDisabled()">利用規約に同意</label>
  <input type="submit" name="send" value="{$_pcmt_messages['btn_comment']}" disabled>
  </div>
EOD;
		$form_end = '</form>' . "\n";
	}
	...
	if (! is_page($_page)) {
		...
		$msg    = $_page;
		$link   = "一覧は".make_pagelink($_page, $msg)."を参照";
		$recent = ! empty($count) ? sprintf($_pcmt_messages['msg_recent'], $count) : '';
	}

	if ($dir) {
		return '<div class="pcomment">' .
			'<p>' . $recent . ' ' . $link . '</p>' . "\n" .
				$comments . "\n" .
			$form_start .
				$form .
...
function plugin_pcomment_insert()
{
	...
	page_write($page, $postdata, PLUGIN_PCOMMENT_TIMESTAMP == 1);
