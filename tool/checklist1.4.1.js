...
$(function(){
	...
	var puretext = el.dataset.text;
	var text = encodeURI(el.dataset.text);
	...
	function maker1(){
		...
		if ( checkLength != 0) {// 1つ以上選択している
			...
			$('textarea[id="sharetext"]').val(puretext + checkRate + '％（所有数' + checkLength + '）http://pad.rtwiki.net/tool/' + file + '?' + compressIdList + ' #rtwiki_net #pzdr');
			$('#shareTW a').attr('href', 'https://twitter.com/intent/tweet?data-related=pkg_rtwiki&related=pkg_rtwiki&text=' + text + checkRate + '%EF%BC%85%EF%BC%88%E6%89%80%E6%9C%89%E6%95%B0' + checkLength + '%EF%BC%89+http://pad.rtwiki.net/tool/' + file + '?' + compressIdList + '%20%23rtwiki_net%20%23pzdr');
